...
void track_gaze(Mat& frame_bgr, Mat& frame_gray) {
	...
	cv::Mat x = frame_bgr(eye0_roi);
	cv::Mat y = frame_bgr(eye1_roi);
	erase_specular(x);
	erase_specular(y);

	// Find relative eye-centers for each eye-roi
	Point c0 = find_eye_center(x);
	Point c1 = find_eye_center(y);
...
Point2i tl_c0 = eye0_roi.tl() + c0;
Point2i tl_c1 = eye1_roi.tl() + c1;
Point2i tl0 = eye0_roi_ref.tl();
Point2i tl1 = eye1_roi_ref.tl();
	Mat eye0_eyelid = get_upper_eyelid(tl_c0, eye_cs_vec, x);
	Mat eye1_eyelid = get_upper_eyelid(tl_c1, eye_cs_vec, y);
	poss_limb_pts_0 = filter_poss_limb_pts(poss_limb_pts_0, eye0_eyelid, tl0);
	poss_limb_pts_1 = filter_poss_limb_pts(poss_limb_pts_1, eye1_eyelid, tl1);
